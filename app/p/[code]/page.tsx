...
type IdStatus = "idle" | "checking" | "valid" | "invalid" | "submitted";
...
export default function ProjectLandingPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`/api/p/${code}/entries/check?studentId=${encodeURIComponent(id)}`, {
          cache: "no-store",
        });
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(`pitching_${code}_name`, data.name || id);
      sessionStorage.setItem(`pitching_${code}_studentId`, id);
      if (data.sessionId) sessionStorage.setItem(`pitching_${code}_sessionId`, data.sessionId);
    ...
    return (
      <StudentBackground>
        <main className="flex items-center justify-center px-4 py-10 min-h-screen">
          <p className="text-slate-500">ไม่พบโปรเจกต์นี้ กรุณาตรวจสอบลิงก์อีกครั้ง</p>
        </main>
      </StudentBackground>
    );
  }
        ...
        <div className="w-full max-w-md bg-white/80 backdrop-blur-sm border border-white/60 rounded-2xl p-6 sm:p-8 shadow-xl shadow-orange-100">
          ...
          {alreadySubmitted && (
            <div className="mb-4 rounded-lg bg-yellow-50 border border-yellow-300 text-yellow-700 text-sm px-3 py-2">
              ...
              <a href={`/p/${code}/board`} className="underline font-medium">
                ไปหน้ากระดานผลงาน
              </a>
            </div>
          )}
          ...
          <p className="text-center text-slate-400 text-sm mt-4">
            หรือ{" "}
            <a href={`/p/${code}/board`} className="underline">
              ดูกระดานผลงาน
            </a>
          </p>
        </div>
